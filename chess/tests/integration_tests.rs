use chess_lib::{
    ...
    pieces::{
        ...
        King, Pawn, Piece, PieceFactory, Queen, Rook,
    },
};
...
fn test_horizontal_right<T: Piece + 'static>(
    game: &mut Game,
    max: i32,
    maybe_blocker: Option<(Position, Color)>,
    clean: bool,
...
fn test_horizontal_left<T: Piece + 'static>(
    game: &mut Game,
    max: i32,
    maybe_blocker: Option<(Position, Color)>,
    clean: bool,
...
fn test_vertical_forward<T: Piece + 'static>(
    game: &mut Game,
    max: i32,
    maybe_blocker: Option<(Position, Color)>,
    clean: bool,
...
fn test_vertical_backward<T: Piece + 'static>(
    game: &mut Game,
    max: i32,
    maybe_blocker: Option<(Position, Color)>,
    clean: bool,
...
fn horizontal_vertical_movements<T: Piece + 'static, F>(
    game: &mut Game,
    max_dimension_x: i32,
    max_dimension_y: i32,
    min_dimension: i32,
    ...
    G: Fn(Result<(), MovementError>, &mut Game),
{
    (1..=max_dimension_x).into_iter().for_each(|i| {
        ...
        if i != max_dimension_x {
            ...
            assert_blocked_target(
                test_horizontal_right::<T>(game, max_dimension_x, blocker, false),
                game,
            );
            assert_ok!(game.board.clean());
        }
    ...
    (0..=max_dimension_x - 1).into_iter().for_each(|i| {
        let blocker = Some((Position { x: i, y: 0 }, blocker_color));
        if i != 0 {
            assert_blocked_path(test_horizontal_left::<T>(
                game,
                max_dimension_x,
                blocker,
                true,
            ...
            assert_blocked_target(
                test_horizontal_left::<T>(game, max_dimension_x, blocker, false),
                game,
            );
            assert_ok!(game.board.clean());
        }
    ...
    (1..=max_dimension_y).into_iter().for_each(|i| {
        ...
        if i != max_dimension_y {
            assert_blocked_path(test_vertical_forward::<T>(
                game,
                max_dimension_y,
                blocker,
                true,
            ...
            assert_blocked_target(
                test_vertical_forward::<T>(game, max_dimension_y, blocker, false),
                game,
            );
            assert_ok!(game.board.clean());
        }
    ...
    (0..=max_dimension_y - 1).into_iter().for_each(|i| {
        let blocker = Some((Position { x: 0, y: i }, blocker_color));
        if i != 0 {
            assert_blocked_path(test_vertical_backward::<T>(
                game,
                max_dimension_y,
                blocker,
                true,
            ...
            assert_blocked_target(
                test_vertical_backward::<T>(game, max_dimension_y, blocker, false),
                game,
            );
            assert_ok!(game.board.clean());
        }
...
fn horizontal_vertical_path_blocked_for_both_colors<T: Piece + 'static>(
    ...
    horizontal_vertical_path_blocked::<T, _, _>(
        ...
        blocker_piece_color,
        |res| {
            assert_err!(res, MovementError::BlockedPath);
        },
        |res, _| {
            assert_err!(res, MovementError::BlockedPath);
        },
    ...
    horizontal_vertical_path_blocked::<T, _, _>(
        ...
        |res, game_after| {
            ...
            assert!(rival_dead_pieces_after
                .last()
                .expect("There is a piece")
                .as_any()
                .downcast_ref::<T>()
                .is_some());
        },
...
fn test_diagonal_forward_right<T: Piece + 'static>(
    game: &mut Game,
    max: i32,
    maybe_blocker: Option<(Position, Color)>,
    clean: bool,
...
fn test_diagonal_forward_left<T: Piece + 'static>(
    game: &mut Game,
    max: i32,
    maybe_blocker: Option<(Position, Color)>,
    clean: bool,
...
fn test_diagonal_backward_right<T: Piece + 'static>(
    game: &mut Game,
    max: i32,
    maybe_blocker: Option<(Position, Color)>,
    clean: bool,
...
fn test_diagonal_backward_left<T: Piece + 'static>(
    game: &mut Game,
    max: i32,
    maybe_blocker: Option<(Position, Color)>,
    clean: bool,
...
fn diagonal_movements<T: Piece + 'static, F>(
    game: &mut Game,
    max_dimension_x: i32,
    max_dimension_y: i32,
    min_dimension: i32,
    ...
    G: Fn(Result<(), MovementError>, &mut Game),
{
    ...
    (1..=max_dimension).into_iter().for_each(|i| {
        ...
        if i != max_dimension {
            assert_blocked_path(test_diagonal_forward_right::<T>(
                game,
                max_dimension,
                blocker_position,
                true,
            ...
            assert_blocked_target(
                test_diagonal_forward_right::<T>(game, max_dimension, blocker_position, false),
                game,
            );
            assert_ok!(game.board.clean());
        }
    ...
    (1..=max_dimension).into_iter().for_each(|i| {
        ...
        if i != max_dimension {
            assert_blocked_path(test_diagonal_forward_left::<T>(
                game,
                max_dimension,
                blocker_position,
                true,
            ...
            assert_blocked_target(
                test_diagonal_forward_left::<T>(game, max_dimension, blocker_position, false),
                game,
            );
            assert_ok!(game.board.clean());
        }
    ...
    (1..=max_dimension).into_iter().for_each(|i| {
        ...
        if i != max_dimension {
            assert_blocked_path(test_diagonal_backward_left::<T>(
                game,
                max_dimension,
                blocker_position,
                true,
            ...
            assert_blocked_target(
                test_diagonal_backward_left::<T>(game, max_dimension, blocker_position, false),
                game,
            );
            assert_ok!(game.board.clean());
        }
    ...
    (1..=max_dimension).into_iter().for_each(|i| {
        ...
        if i != max_dimension {
            assert_blocked_path(test_diagonal_backward_right::<T>(
                game,
                max_dimension,
                blocker_position,
                true,
            ...
            assert_blocked_target(
                test_diagonal_backward_right::<T>(game, max_dimension, blocker_position, false),
                game,
            );
            assert_ok!(game.board.clean());
        }
...
fn rook_movements() {
    ...
    diagonal_movements::<Rook, _>(
        ...
        min_dimension,
        |res| {
            assert_err!(res, MovementError::IllegalMovement);
        },
    );
}
...
fn king_movements_path_blocked() {
    ...
    horizontal_vertical_path_blocked_for_both_colors::<King>(
        &mut game,
        max_dimension_x,
        max_dimension_y,
    );
